<x-main>
    <div class="px-2 mt-20 sm:px-20 sm:px-40">
        <h1 class="pb-10 text-3xl font-semibold text-center">Create New Material</h1>
        <form class="w-full" action="{{ route("post.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <textarea id="body" value="{{ old("body") }}" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Description your learning material...?" name="body"></textarea>
                @error("body")
                     <p class="text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
        <button type="submit" class="text-white bg-[#AACB73] hover:bg-[#70874c] focus:ring-4 focus:outline-none focus:ring-[#ccf58b] font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Submit</button>
        </form>                             
